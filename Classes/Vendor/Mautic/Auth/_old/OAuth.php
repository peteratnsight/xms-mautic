...
class OAuth extends ApiAuth implements AuthInterface
{
     ...
     * @return $this
     */
    public function enableDebugMode($lvl=1)
    {
        $this->_debug = $lvl;
	...
	//$this->log(print_r($this,-1));
		...
		$this->log('still validating and going for requestAccessToken');
	...
	$this->log('Request Settings: '.print_r($settings,1)); 
	$this->log('Request URL: '.$this->_request_token_url);
		...
		die('Incorrect access token parameters returned: '.print_r($response,-1));
	...
	$this->log("Session oauth: ".print_r($_SESSION["oauth"],-1));
	$this->_parent->storeSession($_SESSION["oauth"]);
    ...
    public function makeRequest($url, array $parameters = array(), $method = 'GET', array $settings = array())
    {
        ...
        $this->log('Request Params: '.print_r($parameters,1));
	    ...
	    $this->log('Base Info: '.print_r($base_info,-1));
	    $this->log('Composite: '.$composite_key);
	    $this->log('Request Header: '.print_r($headers,-1));
	...
	$this->log('CURL Options: '.print_r($options,-1));
	...
	$this->log("have curled: ".$body);
        $responseGood = false;

        //Check to see if the response is JSON

        $parsed = json_decode($body, true);
	$this->log("Parsed Return Data: ".print_r($parsed,1));
     ...
     */
    protected function log($message,$loglvl=0)
    {
	switch($this->_debug) {
	  case(2):
	...
	if($this->_debug>0 && $loglvl>2)
		die("die while debugging ....");
